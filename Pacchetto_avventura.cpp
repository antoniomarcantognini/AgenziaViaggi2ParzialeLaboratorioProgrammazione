#include "magic_enum.hpp"
    ...
    namespace {
        ...
        bool inserimento_dati_cliente(string& nome, string& cognome, string& email, string& telefono,
        ...
        bool inserimento_dati_prenotazione(shared_ptr<Cliente>& cliente, shared_ptr<Pacchetto_viaggio>& pacchetto_viaggio,
    ...
    bool Gestore_azienda::carica_pacchetti(std::ifstream& file) {
            ...
            while (getline(file, linea)) {
                ...
                auto temp = mappa_caricamento_specifico.find(tipologia) // Cerco la giusta funzione di caricamento
                if (temp != mappa_caricamento_specifico.end()) return temp->second(codice,destinazione,giorni,disponibile,prezzo,tipologia,campi,numero_riga); // Se non ci sono errori, applico la funzione di caricamento dati corretta
    ...
    bool Gestore_azienda::aggiungiPacchetto() {
        do{
            string codice; string dest; int giorni; doule prezzo;
            // Inserimento interattivo dei dati del pacchetto di base
            inserisci_dati_pacchetto_base(dest,giorni,prezzo);
            codice = generaCodiceUnico('P');
            ...
            switch (scelta) {
                case 1:
                    ...
                    } else flag = crea_elemento<Pacchetto_avventura>(codice,dest,giorni,prezzo,lista_attivita,categoria,assicurazione); // Creazione dell'elemento
                    break;
                ...
                case 2:
                    ...
                    } else flag = crea_elemento<Pacchetto_mare>(codice, dest, giorni, prezzo, ombrellone, attrezzatura, tipo); // Creazione dell'elemento
                    break;
                ...
                case 3: 
                    ...
                    } else flag = crea_elemento<Pacchetto_montagna>(codice, dest, giorni, prezzo, skipass_incluso, num_escursioni, difficolta); // Creazione dell'elemento
                    break;

                default: // scelta deve essere necessariamente 4
                    int num_musei; bool guida; Categoria_hotel hotel;
                    if (!inserisci_dati_pacchetto(num_musei, guida, hotel)) { // Errore -> reinserimento?
                        int scelta = menu::menu_reinserimento();
                        if (scelta == 2) return false;
                        else continue;
                    } else flag = crea_elemento<Pacchetto_citta>(codice, dest, giorni, prezzo, num_musei, guida, hotel); // Creazione dell'elemento
                    break;
            }
    ...
    bool Gestore_azienda::visualizzaPacchettiPerTipologia(string tipo) const {
        ...
        if (!flag) {cerr << "Errore! Non ci sono elementi della tipologia inserita!" << endl; return false;}
        return true;
    }
    ...
    bool Gestore_azienda::aggiungiPrenotazione() {
        ...
        do {
            ...
            if (!inserimento_dati_prenotazione(cliente, pacchetto_viaggio, num_persone, data)) {
                int scelta = menu::menu_reinserimento();
                if (scelta == 2) return false; // Se scelta è 2 l'utente ha scelto di annullare l'operazione
    ...
    bool Gestore_azienda::visualizzaPrenotazioniCliente(string codice_cliente) const {
        int i = 0;
        ...
        cout << "Cliente con codice " << codice_cliente << "non trovato";
        return false;
    }
