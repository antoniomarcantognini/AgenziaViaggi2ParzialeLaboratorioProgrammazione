...
string Pacchetto_avventura::stampa_dettagli() const {
    ...
    // Nota: usa i getter della classe base se le variabili sono private, oppure this->variabile se protected
    ...
    // Dettagli specifici
    // Usa enum_name di magic_enum per stampare la stringa dell'enum
    ss << "Livello Adrenalina: " << enum_name(categoria_adrenalina) << endl;
...
// Metodi statici di conversione (Legacy, se non usi magic_enum ovunque)
...
bool Pacchetto_avventura::salva_dati_su_file(ofstream& file) const {
    ...
    file << enum_name(this->categoria_adrenalina) << ";"
    ...
    // cout << "Pacchetto Avventura " << this->get_codice_pacchetto() << " salvato correttamente." << endl;
    return true;
}
