...
string Pacchetto_mare::stampa_dettagli() const {
    ...
    // Dettagli specifici
    ss << "Ombrellone incluso: " << (ombrellone_incluso ? "Si" : "No") << endl;
    ss << "Attrezzatura Sportiva: " << (attrezzatura_sportiva ? "Si" : "No") << endl; 
    ss << "Trattamento: " << Utils_enum::etos(tipologia) << endl;
...
bool Pacchetto_mare::salva_dati_su_file(ofstream& file) const {
    ...
    cout << "Pacchetto Mare " << this->get_codice() << " salvato correttamente." << endl;
    return true;
}